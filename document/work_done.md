...
## Update
Timestamp: 2026-04-13 13:27:11 +0530

### Phase 7 UI/UX Polish - Empty State Pass

- Upgraded the shared `EmptyState` UI primitive so it now owns icon rendering, uses component-based SVG icons with inherited color, applies a softer illustration treatment, reserves more vertical space, and adds a motion-safe fade-in animation for smoother empty-result rendering.
- Updated all current empty-result pages to use the stricter shared icon API, covering `/vehicles`, `/log`, `/admin/vehicles`, `/admin/members`, and `/admin/requests`.
...
## Update
Timestamp: 2026-04-16 17:52:56 +0530

### Mobile-First Schedule / Calendar / Booking Redesign

- Added a shared `MobileBackHeader`, `ChevronLeftIcon`, and `mobile-sticky-subnav` treatment so the schedule, booking, and privileges flows can use a compact mobile back/title pattern and sticky mobile segmented controls without changing any routes or server actions.
- Rebuilt `/vehicles/[vehicleId]/calendar` around a mobile-first layout: removed the mobile breadcrumb/stat-card wall, moved straight into a compact month navigator, replaced the tall day capsules with a tighter month grid, simplified each day to confirmed/requested dot signals plus compact overflow counts, and added a non-modal inline selected-day action sheet that keeps the user on the calendar until they choose `Continue booking`.
- Simplified `/vehicles/[vehicleId]/date/[date]` and `BookingWorkspace` for phones: replaced breadcrumb-heavy mobile chrome with the compact back header plus context strip, removed the repeated booking summary card, made the Timeline/Form switch sticky and mobile-default to `Form`, converted `All-day booking` into a compact switch row, swapped the time fields to native `type="time"` inputs while keeping the same `start_time` / `end_time` payloads, and kept the primary submit in the sticky mobile action area.
- Flattened `/admin/privileges` on mobile so the page now shows the title, a compact current-policy strip, then grouped `Booking behavior` and `Rules` sections with the existing sticky save action; the existing singleton config query, field names, and `updatePrivileges` action remain unchanged.
- Kept all booking/calendar/admin data contracts unchanged: the calendar and booking routes are the same, Supabase queries remain route-local, booking creation still posts `is_all_day`, `start_time`, `end_time`, and `reason`, and no schema or validation logic changed.
- Verified this pass with `npx tsc --noEmit --pretty false`, `npm test`, and `npm run build`.
- Attempted `npm run lint`, but `eslint` stalled again in this WSL/Windows-mounted workspace and did not complete, so lint could not be fully confirmed in the current environment.
- Full manual visual QA for the redesigned schedule/calendar/booking flow at `360x800` / `390x844` plus desktop regression on the same routes was not completed from this terminal-only environment and remains the next validation step.
